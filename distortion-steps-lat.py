'''
Calculates distortion in distance of a projection by stepping through different latitudes
'''
...
def distortion(lat1, lon1, lat2, lon2, epsg):
    ...
    print("{} \t {} %".format(lat1, abs(1-distGC/distProj)*100))
...
if __name__== "__main__":
    ...
    lon2 = 42.0
    lat = -85.0
    epsg = "+proj=stere +lat_0=90 +lat_ts={} +lon_0=0 +k=1 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs"

    # step through latitudes
    while lat<90.0:
        if lat != 0.0:
            distortion(lat, lon1, lat, lon2, epsg)
        lat = lat+5.0
